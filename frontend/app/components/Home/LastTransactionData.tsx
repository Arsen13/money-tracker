...
export default function LastTransactionData() {
  ...
  return (
    <div className='flex justify-around'>
      <div className='flex flex-col items-center gap-0.5 rounded-xl bg-linear-to-b from-cyan-500 to-blue-500 px-7 py-0.5'>
        <p className='text-sm'>Income</p>
        <p className='text-2xl'>{lastTransaction?.income.amount || 0}</p>
        <p className='text-sm italic'>
          {lastTransaction?.income.category?.toLowerCase() || ''}
        </p>
      </div>
      <div className='flex flex-col items-center gap-0.5 rounded-xl bg-linear-to-b from-violet-500 to-fuchsia-500 px-7 py-0.5'>
        <p className='text-sm'>Expense</p>
        <p className='text-2xl'>{lastTransaction?.expense.amount || 0}</p>
        <p className='text-sm italic'>
          {lastTransaction?.expense.category?.toLowerCase() || ''}
        </p>
      </div>
    </div>
  );
